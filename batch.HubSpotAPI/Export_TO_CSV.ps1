...
Invoke-Sqlcmd -ServerInstance V-AWD-SQL-09 -querytimeout 0 -Query "SELECT Brand,LifeCycleStage,ClientCode,VID,Email,IsNew,Status,REPLACE(Error_Msg,CHAR(10),CHAR(32)),LogDate FROM [HUBSPOT].[dbo].[ST_ERROR_LOG] WITH (NOLOCK) WHERE Status='Fail' ORDER BY LogDate;" | Export-Csv -NoTypeInformation $($expLocation + "HUBSPOT_API_Errors_" + $currdate + ".csv")
